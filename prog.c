...
/*From http://lionet.info/asn1c/examples.html */
int main() {
    ...
    assert(myInt); *myInt = 123;
    /* Set integer value */
    /* Fill in mySeqOf with the prepared INTEGER */
    ret = ASN_SEQUENCE_ADD(&myType->mySeqOf, myInt);
    assert(ret == 0); /* Prepare another integer */
    myInt = calloc(1, sizeof *myInt);
    assert(myInt); *myInt = 111222333;
    /* Set integer value */
